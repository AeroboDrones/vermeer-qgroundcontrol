...
void VermeerFirebaseManager::fetchFlightPlansReadyRead()
{
    ...
    bool isValidFetch = !replyJsonObj.contains("error");
...
{
    isXavierConnected = false;
    emit(displayMsgToQml(QVariant("xavier_disconnected")));
}
...
void VermeerFirebaseManager::sendHeartbeatMsg(QVariant ipaddress, QVariant portNumber)
{
    QJsonObject heartbeatMsgJson;
    if(false == isXavierConnected) {
        heartbeatMsgJson["heartbeat"] = "disconnected";
    } else if(true == isXavierConnected) {
    ...
    QNetworkDatagram datagram(heartbeatMsgByteArray,QHostAddress(ipaddress.toString()),portNumber.toInt());
    socket.writeDatagram(datagram);
}
